...
sub renderBreadCrumbs {
    ...
    foreach my $item (@$breadCrumbs) {
        ...
        $webtopic =~ s/\//./go;
        $item->{name} = spaceOutWikiWord( $item->{name}, $spaceoutsep )
          if $spaceout;
...
sub getTopicTitle {
    ...
    if ( Foswiki::Func::getContext()->{'DBCachePluginEnabled'} ) {
        ...
        return Foswiki::Plugins::DBCachePlugin::getTopicTitle( $theWeb,
            $theTopic );
    }
    ...
    if ( $Foswiki::cfg{SecureTopicTitles} ) {
        ...
        return $theTopic
          unless Foswiki::Func::checkAccessPermission( 'VIEW', $wikiName, $text,
            $theTopic, $theWeb, $meta );
    }
